-- Migration 042: tour_image
-- Добавляет поле tour_image в таблицу tours
-- Обновляет v_route_marketplace для передачи изображения в OfferCard

-- 1. Добавить колонку tour_image в tours
ALTER TABLE tours
  ADD COLUMN IF NOT EXISTS tour_image TEXT;

-- 2. Пересоздать v_route_marketplace с tour_image
...
COMMENT ON VIEW v_route_marketplace IS
  'Маршруты + туры + операторы + ближайший заезд. Migration 042: добавлено tour_image и operator_hero_image.';
