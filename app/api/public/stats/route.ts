...
export async function GET() {
  try {
    const [routesResult, toursResult, partnersResult, reviewsResult] = await Promise.all([
      query<{ total: number }>('SELECT COUNT(*)::int AS total FROM kamchatka_routes'),
      query<{ total: number }>('SELECT COUNT(*)::int AS total FROM tours WHERE is_active = true'),
      query<{ total: number }>('SELECT COUNT(*)::int AS total FROM partners WHERE is_verified = true'),
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        verifiedPartners: partnersResult.rows[0]?.total ?? 0,
