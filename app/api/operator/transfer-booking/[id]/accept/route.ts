...
export async function PATCH(
  ...
  try {
    ...
    const bookingResult = await query<{ total_price: string; status: string; tour_id: string }>(
      `SELECT COALESCE(final_price, base_total_price) AS total_price, booking_status AS status, operator_tour_id AS tour_id FROM operator_bookings WHERE id = $1 LIMIT 1`,
      [transfer.booking_id]
    );
