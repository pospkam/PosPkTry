  ...
  try {
    ...
    const overviewResult = await query<OpAnalyticsOverviewRow>(
      `SELECT
        ...
        COALESCE(SUM(CASE WHEN b.payment_status = 'paid' THEN COALESCE(b.final_price, b.base_total_price) ELSE 0 END), 0) as paid_revenue,
      ...
      LEFT JOIN operator_bookings b ON t.id = b.operator_tour_id AND b.created_at >= $2
    ...
    const topToursResult = await query<OpAnalyticsTopTourRow>(
      ...
      FROM operator_tours t
      LEFT JOIN operator_bookings b ON t.id = b.operator_tour_id AND b.created_at >= $2 AND b.booking_status != 'cancelled'
    ...
    const conversionResult = await query<OpAnalyticsConversionRow>(
      `SELECT
        COUNT(*) FILTER (WHERE b.booking_status = 'pending') as pending,
