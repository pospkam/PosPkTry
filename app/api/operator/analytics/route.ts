  ...
  try {
    ...
    const { rows: revenueData } = await pool.query<{
      ...
      `SELECT
       ...
       WHERE ob.operator_id = $1 AND tp.paid_at >= $2 AND tp.status = 'RELEASED'
       GROUP BY DATE_TRUNC('month', tp.paid_at)
       ORDER BY month DESC`,
      [operatorId, startDate.toISOString()]
    );
    ...
    const { rows: conversionData } = await pool.query<{
      ...
      `SELECT
         ...
         (SELECT COUNT(*) FROM operator_bookings ob
          WHERE ob.operator_id = $1 AND ob.created_at >= $2) as total_bookings,
         CASE
           ...
           THEN ROUND(
             (SELECT COUNT(*) FROM operator_bookings ob
              WHERE ob.operator_id = $1 AND ob.created_at >= $2)::numeric /
             (SELECT COUNT(*) FROM page_views pv
              JOIN operator_tours ot ON pv.path LIKE '%/routes/' || ot.agent_route_id || '%'
    ...
    const { rows: summary } = await pool.query<{
      ...
      `SELECT
         ...
         (SELECT COUNT(*) FROM operator_bookings ob2
          WHERE ob2.operator_id = $1 AND ob2.booking_status = 'completed'
          AND ob2.created_at >= $2) as completed_bookings
       FROM operator_bookings ob
       LEFT JOIN tour_payments tp ON tp.booking_id = ob.id AND tp.status = 'RELEASED'
       WHERE ob.operator_id = $1 AND ob.created_at >= $2`,
      [operatorId, startDate.toISOString()]
    );
