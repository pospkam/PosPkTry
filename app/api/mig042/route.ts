...
export async function POST(request: NextRequest) {
  ...
  try {
    await query(`ALTER TABLE tours ADD COLUMN IF NOT EXISTS tour_image TEXT`);

    await query(`DROP VIEW IF EXISTS v_route_marketplace`);
    ...
    return NextResponse.json({ success: true, message: 'Migration 042 applied: tour_image added, v_route_marketplace updated' });
  } catch (error) {
