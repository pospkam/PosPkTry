  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 transition-all hover:scale-105">
      ...
      <div className="relative h-48 bg-white/10">
        ...
        {!souvenir.inStock && (
          <div className="absolute top-2 right-2 px-3 py-1 bg-red-500/90 text-white text-xs rounded-full">
            Нет в наличии
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        <div className="mb-2">
          <span className="text-xs text-white/50">{souvenir.category}</span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-2xl font-black text-premium-gold">
              {souvenir.price.toLocaleString('ru-RU')} ₽
            </p>
          </div>

          <button
            onClick={() => onAddToCart(souvenir.id)}
            disabled={!souvenir.inStock}
            className="px-4 py-2 bg-premium-gold hover:bg-premium-gold/80 text-premium-black font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
