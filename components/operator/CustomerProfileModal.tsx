...
export default function CustomerProfileModal({ clientId, onClose, onTagsUpdated }: Props) {
  ...
  return (
    ...
    >
      <div className="bg-gray-900 border border-white/15 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
          ...
          ) : <span className="text-white">Профиль клиента</span>}
            ...
            <>
              ...
              <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {tags.filter(t => !PRESET_TAGS.includes(t)).map(t => (
                      <span key={t} className="flex items-center gap-1 px-2.5 py-1 bg-purple-500/20 border border-purple-500/30 rounded-full text-xs text-purple-300">
                        {t}
                        <button onClick={() => removeTag(t)} className="hover:text-white ml-0.5">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    maxLength={20}
                    className="flex-1 min-h-[36px] px-3 py-1.5 bg-white/5 border border-white/15 rounded-lg text-sm text-white placeholder-white/30 focus:outline-none focus:border-cyan-500/50"
                  />
                  <button
              ...
              <div>
                ...
                {profile.bookings.length === 0 ? (
                  <p className="text-sm text-white/30 py-3 text-center">Нет бронирований</p>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {profile.bookings.map(b => {
                          const s = STATUS_MAP[b.status] ?? { label: b.status, cls: 'bg-white/10 text-white/50' };
                          return (
                            <tr key={b.id} className="border-b border-white/5 last:border-0">
                              <td className="p-3 text-white/80 truncate max-w-[160px]">{b.tourName}</td>
                              <td className="p-3 text-white/50 whitespace-nowrap">{fmtDate(b.createdAt)}</td>
                              <td className="p-3">
                                <span className={`text-xs px-2 py-0.5 rounded-full ${s.cls}`}>{s.label}</span>
                              </td>
                              <td className="p-3 text-right text-white/80 whitespace-nowrap font-medium">{fmt(b.totalPrice)} ₽</td>
                            </tr>
                          );
                        })}
                      </tbody>
                    ...
                    {profile.reviews.map(r => (
                      <div key={r.id} className="bg-white/5 border border-white/10 rounded-xl p-3">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-white/50">{r.tourName}</span>
                          <div className="flex items-center gap-0.5">
                            {[1,2,3,4,5].map(s => (
                              <Star key={s} className={`w-3 h-3 ${s <= r.rating ? 'text-amber-400 fill-amber-400' : 'text-white/20'}`} />
                            ))}
                          </div>
                        </div>
                        {r.comment && <p className="text-sm text-white/70">{r.comment}</p>}
                        <p className="text-xs text-white/30 mt-1">{fmtDate(r.createdAt)}</p>
                      </div>
                    ))}
                  </div>
                </div>
